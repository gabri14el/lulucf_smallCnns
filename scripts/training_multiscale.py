...
experiment_name = 'Baseline3DExtendedExperimentMS'
...
def pre_process(X):
    return np.expand_dims(X, axis=-1)/max
...
y = tf.keras.layers.Conv3D(filters=32, kernel_size=(2, 2, 1), activation="relu", padding="same")(_input)
#z = tf.keras.layers.Conv3D(filters=64, kernel_size=5, activation="gelu", padding="same")(_input)
m = tf.keras.layers.Conv3D(filters=32, kernel_size=(1,1, 5), activation="relu", padding="same")(_input)
conc = tf.keras.layers.Concatenate(axis=-1)([x, y,m])
x = tf.keras.layers.Conv3D(filters=32, kernel_size=3, activation="relu", padding="same")(conc)
